...
CHANNEL_RULES_PATH = BASE_DIR / "channel_rules.md"
DIALOGUE_EXAMPLES_PATH = BASE_DIR / "dialogue_examples.md"
CUSTOMER_DIALOGUE_TRAINING_PATH = BASE_DIR / "customer_dialogue_training.md"
...
Если данных недостаточно, попроси фото или модель: Пришлите фото/модель — подскажу точнее.
"""
...
Bauer Vertexx — это вратарские стаканы Bauer. Если клиент пишет "вратарские Bauer", "Bauer goalie", "Vertexx", "Вертекс", то речь, скорее всего, про Bauer Vertexx.
...
FALLBACK_CHANNEL_RULES = """# Правила ответов по каналам

Если канал неизвестен, отвечай коротко и безопасно: не проси оплату на карту, номер телефона и не уводи клиента в другой мессенджер. Для avito строго соблюдай правила площадки.
"""
...
FALLBACK_CUSTOMER_DIALOGUE_TRAINING = """# Обучение агента на клиентских переписках

Этот файл является рабочим обучающим материалом. Не использовать сырой текст как прямой источник ответа клиенту.
"""
...
ALLOWED_CHANNELS = {"telegram", "avito", "whatsapp", "instagram", "website", "unknown"}
AVITO_SAFE_REPLY = (
    "На Авито безопаснее проводить оплату и оформление через доступные инструменты площадки. "
    "Можем посчитать вариант через Авито Доставку и подсказать по наличию."
)
AVITO_FORBIDDEN_PATTERNS = (
    r"\bсбер\b",
    r"\bкарт[ауыое]\b",
    r"\bперевод\b",
    r"\bпереведите\b",
...
def normalize_channel(value: str | None) -> str:
    ...
    return normalized if normalized in ALLOWED_CHANNELS else "unknown"


def find_channel_in_payload(payload: dict[str, Any] | None) -> str:
    if not isinstance(payload, dict):
        return "unknown"

    for key in ("channel", "source", "platform"):
        channel = normalize_channel(str(payload.get(key))) if payload.get(key) else "unknown"
        if channel != "unknown":
            return channel

    for key in ("payload", "value", "message", "meta"):
        nested = payload.get(key)
        if isinstance(nested, dict):
            channel = find_channel_in_payload(nested)
            if channel != "unknown":
                return channel
...
def infer_channel(request: Request, payload: dict | None = None) -> str:
    path = request.url.path.lower()
    if path.endswith("/webhook/telegram"):
        return "telegram"
    if path.endswith("/webhook/avito"):
        return "avito"

    channel = find_channel_in_payload(payload)
    return channel if channel != "unknown" else "unknown"


def build_system_prompt(channel: str = "unknown") -> str:
    ...
    channel_rules = read_text_or_fallback(CHANNEL_RULES_PATH, FALLBACK_CHANNEL_RULES)
    dialogue_examples = read_text_or_fallback(DIALOGUE_EXAMPLES_PATH, FALLBACK_DIALOGUE_EXAMPLES)
    customer_dialogue_training = read_text_or_fallback(
        CUSTOMER_DIALOGUE_TRAINING_PATH,
        FALLBACK_CUSTOMER_DIALOGUE_TRAINING,
    )
    ...
    return (
        f"{system_prompt}\n\n"
        f"## Текущий режим\n\n{mode}\n\n"
        f"## Правила канала общения\n\n"
        f"Канал текущего диалога: {channel}\n\n"
        f"{channel_rules}\n\n"
        f"Агент обязан учитывать канал. Если канал avito, соблюдать ограничения Авито.\n\n"
        f"## База знаний\n\n{knowledge}\n\n"
        f"## Готовые сценарии\n\n{scripts}\n\n"
        f"## Текущие акции и специальные предложения\n\n{offers}\n\n"
        f"## Примеры клиентских диалогов\n\n{dialogue_examples}\n\n"
        f"## Рабочие клиентские переписки для обучения\n\n"
        f"Этот блок не является прямой базой знаний. Используй его только как материал для выявления сценариев. "
        f"Не копируй персональные данные и сырой хаос переписки в ответы клиенту.\n\n"
        f"{customer_dialogue_training}"
    )
...
def sanitize_reply_for_channel(reply: str, channel: str) -> str:
    if normalize_channel(channel) != "avito":
        return reply

    lowered = normalize_text(reply)
    if PHONE_RE.search(reply):
        return AVITO_SAFE_REPLY

    for pattern in AVITO_FORBIDDEN_PATTERNS:
...
def blade_options_reply(holder: str, size: str) -> str:
    budget_price = price_for("budget", holder)
    base_price = price_for("base", holder)
    pro_price = price_for("pro", holder)
    if budget_price is None or base_price is None or pro_price is None:
        return price_missing_reply()

    return (
        f"Понял, нужны лезвия {holder} {size}.\n\n"
        "Есть три варианта:\n"
        f"1. Бюджетные лезвия без бренда — базовая цена {budget_price} ₽.\n"
        f"2. in hockey® Base — базовая цена {base_price} ₽.\n"
        f"3. in hockey® Pro — базовая цена {pro_price} ₽.\n\n"
...
def budget_blade_reply(holder: str, size: str) -> str:
    ...
    if price is None:
        return price_missing_reply()
    return (
        f"Самый доступный вариант для {holder} {size} — бюджетные лезвия без бренда. Базовая цена {price} ₽.\n\n"
...
def local_fallback_reply(
    ...
    if channel == "avito":
        if "авито" in lowered and "достав" in lowered:
            return "Авито Доставка возможна. Цена может отличаться от цены при покупке напрямую, потому что скидки и бонусы UDS там не всегда применяются. Можем посчитать вариант через Авито Доставку."
        if "сдэк" in lowered and any(token in lowered for token in ("далеко", "далек", "ехать")):
            return "Понимаю. Тогда можем рассмотреть Авито Доставку, если она доступна по этому товару. Посчитаем итоговую стоимость и подскажем, как выгоднее."
        if "как оплат" in lowered or "оплатить" in lowered:
            return "На Авито безопаснее проводить оплату через доступные способы самой площадки. Если оформляем Авито Доставку, оплата проходит через Авито."
        if "нельзя" in lowered and ("карт" in lowered or "перев" in lowered):
            return "Авито может ограничивать сообщения, если в них есть данные для оплаты или перехода в другие каналы. Лучше оформить сделку через доступные инструменты Авито."
...
def append_history_and_trim(history_key: str, role: str, content: str) -> None:
    history = chat_histories.setdefault(history_key, [])
    history.append({"role": role, "content": content})
    chat_histories[history_key] = history[-MAX_HISTORY_MESSAGES:]


def generate_reply(buyer_message: str, chat_id: str = "", channel: str = "unknown") -> str:
    channel = normalize_channel(channel)
    ...
    if os.getenv("BOT_MODE", "AUTO").strip().upper() == "MANUAL":
        ...
        reply = sanitize_reply_for_channel(reply, channel)
        append_history_and_trim(history_key, "user", buyer_message)
        append_history_and_trim(history_key, "assistant", reply)
        return reply

    if not ANTHROPIC_API_KEY:
        recent_history = [*history, {"role": "user", "content": buyer_message}][-MAX_HISTORY_MESSAGES:]
        reply = sanitize_reply(local_fallback_reply(buyer_message, recent_history, channel=channel))
    ...
    append_history_and_trim(history_key, "user", buyer_message)
    recent_history = chat_histories[history_key][-MAX_HISTORY_MESSAGES:]
    ...
    try:
        ...
        response = client.messages.create(
            ...
            max_tokens=500,
            temperature=0.2,
            system=build_system_prompt(channel=channel),
            messages=recent_history,
        )
        reply = response.content[0].text
    except Exception:
        logging.exception("LLM reply generation failed")
        reply = local_fallback_reply(buyer_message, recent_history, channel=channel)
...
async def avito_webhook(request: Request) -> dict[str, str]:
    ...
    channel = infer_channel(request, payload)
    ...
    reply = generate_reply(text, chat_id=f"avito_{chat_id}", channel=channel)
...
async def telegram_webhook(request: Request) -> dict[str, str]:
    ...
    reply = generate_reply(text, chat_id=f"tg_{chat_id}", channel=channel)
